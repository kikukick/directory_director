...
function loadLocalTpl(){try{return JSON.parse(localStorage.getItem('localTemplates'))||{}}catch{return {}}}
function saveLocalTpl(obj){localStorage.setItem('localTemplates',JSON.stringify(obj))}
function getAllTemplates(){return Object.assign({},window.TEMPLATES||{},loadLocalTpl())}
function refreshTplSelect(){const all=getAllTemplates();tplSelect.innerHTML='<option value="">テンプレート選択...</option>';for(const key in all){const opt=document.createElement('option');opt.value=key;opt.textContent=key+(loadLocalTpl()[key]?' (自作)':'');tplSelect.appendChild(opt)}}
...
addTplBtn.addEventListener('click',()=>{
  ...
  all[key].files.forEach(f=>{
    addPath(activeFolderNode.querySelector('.children'),f.name,f.content)
  })
})
...
addToTplBtn.addEventListener('click',()=>{
  ...
  if(!key)return
  const local=loadLocalTpl()
  local[key]={files:[{name:activeFileNode.querySelector('.name').value,content:preEditor.textContent}]}
  saveLocalTpl(local)
  refreshTplSelect()
...
function maybeSwitchNode(action){
  ...
  if(curName!==lastSaved.name||curContent!==lastSaved.content){pendingAction=action;confirmModal.style.display='flex';return}
  action()
}
confirmYes.addEventListener('click',()=>{confirmModal.style.display='none';pendingAction&&pendingAction();pendingAction=null})
...
function applySave(){
  ...
  if(isMediaExt(ext)){activeFileNode.querySelector('.name').value=filenameInput.value.trim()+'.'+ext}
  else{activeFileNode.querySelector('.name').value=filenameInput.value.trim();activeFileNode.dataset.content=preEditor.textContent}
}

importBtn.addEventListener('click',()=>{if(!activeFolderNode){showToast('フォルダを選択してください。');return}fileInput.click()})
fileInput.addEventListener('change',e=>{
  for(const file of e.target.files){
    const ext=file.name.split('.').pop().toLowerCase()
    let li
    if(isTextExt(ext)){li=addTextFile(activeFolderNode.querySelector('.children'));const reader=new FileReader();reader.onload=()=>li.dataset.content=reader.result;reader.readAsText(file,'UTF-8')}
    else{li=addNode(activeFolderNode.querySelector('.children'),file.name,false,false);fileBlobs.set(li,{url:URL.createObjectURL(file),ext})}
  }
  fileInput.value=''
})
  ...
  (function walk(ul,dir){
    ul.querySelectorAll(':scope>.node').forEach(li=>{
      ...
      if(li.classList.contains('file')){
        if(fileBlobs.has(li)){fetch(fileBlobs.get(li).url).then(r=>r.blob()).then(b=>dir.file(nm,b))}
        else{dir.file(nm,li.dataset.content||'')}
      } else{const sub=dir.folder(nm);walk(li.querySelector('.children'),sub)}
    })
  })(tree,zip)
...
function addNode(parent,name,isFolder,isRoot){
  ...
  const ctrl=`${isFolder?'<button class="add-folder">📁+</button><button class="add-file">📄+</button>':''}${!isRoot?'<button class="delete">🗑️</button>':''}`
  li.innerHTML=`<input class="name" value="${name}" ${isFolder?'':'readonly'}/><span class="controls">${ctrl}</span><ul class="children"></ul>`
...
function addPath(parent,path,content){
  const parts=path.split('/')
  const leaf=parts.pop()
  let container=parent
  for(const part of parts){
    let folder=Array.from(container.children).find(li=>li.querySelector('.name').value===part&&li.classList.contains('folder'))
    if(!folder)folder=addNode(container,part,true,false)
    container=folder.querySelector('.children')
  }
  const li=addNode(container,leaf,false,false)
  li.dataset.content=content
}

function isTextExt(ext){return['txt','js','html','css','py','java','md','json'].includes(ext)}
function isMediaExt(ext){return['mp3','wav','ogg','mp4','mov','webm','png','jpg','jpeg','gif','h5','keras','pyc','log'].includes(ext)}
...
function drawWaveform(url){
  ...
  fetch(url).then(r=>r.arrayBuffer()).then(buf=>new AudioContext().decodeAudioData(buf)).then(audioBuf=>{
...
document.addEventListener('mousemove',e=>{if(!isDragging)return;const pct=e.clientX/window.innerWidth*100; if(pct>10&&pct<90){document.getElementById('leftPanel').style.width=pct+'%';document.getElementById('rightPanel').style.width=(100-pct)+'%'}})
document.addEventListener('mouseup',()=>{if(isDragging){isDragging=false;document.body.style.cursor=''}})

function showToast(msg){
  const t=document.createElement('div')
  t.textContent=msg
  Object.assign(t.style,{position:'fixed',bottom:'1rem',left:'50%',transform:'translateX(-50%)',background:'#444',color:'#fff',padding:'0.5rem 1rem',borderRadius:'4px',fontSize:'0.9em',zIndex:2000})
